...
function collabBrief(boardId: string, sessionId: string, origin: string): string {
  ...
  return [
    ...
    "empty). As part of the same change: REPLY to what you can answer, RESOLVE what your change settles, and",
    "RE-ANCHOR any surviving open comment whose quoted text your edit moved (op reanchor, a fresh selector",
    "minted against the NEW file text) — an un-re-anchored comment surfaces as a loud orphan strip on the",
    'card, never silently. When asked to "answer the comments on <file>": read them, reply per annotation,',
    'resolve what is settled, and where the right answer is "fix the doc", fix the doc (then re-anchor).',
    "",
