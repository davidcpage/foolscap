...
export class SelectTool implements Tool {
  ...
  onPointerDown(e: PointerInput): void {
    ...
    if (hit) {
      ...
      // e.g. click one member of a group → select just it, so rename / scroll-one works. A cluster the
      // host assembled (a right-clicked thread + its members) is a multi-selection like any other, so
      // this same rule lets a press+drag move the whole group while a plain click narrows to one card.
      const narrowOnUp = !e.shiftKey && this.ctx.selection.has(hit) && this.ctx.selection.size > 1;
      // Selection no longer auto-expands to a host cluster on a plain click: pressing a thread selects
      // the thread alone (the group is assembled deliberately by a RIGHT-click in the app layer). The
      // host's expandSelection resolver survives only for resizeTargetId (which seed a pre-selected
      // cluster resizes from) — the engine stays card-type-blind about what a cluster means.
  ...
  onPointerMove(e: PointerInput): void {
    ...
    switch (s.kind) {
      ...
      case "marquee": {
        ...
        this.ctx.selection.set([...s.base, ...hits]); // base (shift) ∪ rubber-banded — only what the box covers
        break;
      }
    }
  }
